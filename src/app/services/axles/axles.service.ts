...
import { Axle, AxleSpacing, Axles, Empty, VehicleTypes } from '@models/vehicle-tech-record.model';
...
export class AxlesService {
	...
	normaliseAxles(axles?: Axles, axleSpacings?: AxleSpacing[]): [Axles | undefined, AxleSpacing[] | undefined] {
	...
	generateAxlesFromAxleSpacings(vehicleAxleSpacingsLength: number, previousAxles?: Empty<Axle>[]): Empty<Axle>[] {
		...
		return this.fb.group({
			...
			weights_gbWeight: this.fb.control<number | null>(axle?.weights_gbWeight || null, [
				this.commonValidators.max(99999, (control: AbstractControl) => {
					const index = _.indexOf(control.parent?.get('techRecord_axles') as any, control.parent);
					return {
