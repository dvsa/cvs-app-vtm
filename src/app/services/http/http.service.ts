...
export class HttpService {
  ...
  amendTechRecordVin(newVin: string, systemNumber: string, createdTimestamp: string) {
    return this.http.patch<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/updateVin/${systemNumber}/${createdTimestamp}`, {
      newVin,
    });
  }

  amendTechRecordVrm(newVrm: string, isCherishedTransfer: boolean, systemNumber: string, createdTimestamp: string, thirdMark?: string) {
    return this.http.patch<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/updateVrm/${systemNumber}/${createdTimestamp}`, {
      newVrm,
      isCherishedTransfer,
  ...
  archiveTechRecord(systemNumber: string, createdTimestamp: string, reasonForArchiving: string) {
    return this.http.patch<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/archive/${systemNumber}/${createdTimestamp}`, {
      reasonForArchiving,
    });
  }
  ...
  archiveTechRecordStatus(body: TechRecordArchiveAndProvisionalPayload, systemNumber: string) {
    ...
    return this.http.post<CompleteTechRecordPUT>(`${environment.VTM_API_URI}/vehicles/archive/${encodeURIComponent(String(systemNumber))}`, body);
  }
  ...
  fetchRequiredStandards(euVehicleCategory: string) {
    return this.http.get<DefectGETRequiredStandards>(`${environment.VTM_API_URI}/defects/required-standards?euVehicleCategory=${euVehicleCategory}`);
  }
  ...
  getTechRecords(searchIdentifier: string, metadata?: boolean, status?: string, searchCriteria?: string) {
    ...
    return this.http.get<CompleteTechRecords>(`${environment.VTM_API_URI}/vehicles/${encodeURIComponent(String(searchIdentifier))}/tech-records`, {
      params,
    });
  }

  getTechRecordV3(systemNumber: string, createdTimestamp: string) {
    return this.http.get<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/${systemNumber}/${createdTimestamp}`);
  }
  ...
  promoteTechRecord(systemNumber: string, createdTimestamp: string, reasonForPromoting: string) {
    return this.http.patch<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/promote/${systemNumber}/${createdTimestamp}`, {
      reasonForPromoting,
    });
  }
  ...
  referenceResourceTypeGet(resourceType: string, paginationToken?: string) {
    ...
    return this.http.get<ReferenceDataApiResponse>(`${environment.VTM_API_URI}/reference/${encodeURIComponent(String(resourceType))}`, {
      params,
    });
  }
  ...
  referenceResourceTypeResourceKeyDelete(resourceType: string, resourceKey: ResourceKey, body?: Record<string, unknown>) {
    if (resourceType === null || resourceType === undefined) {
  ...
  searchTechRecords(type: SEARCH_TYPES, term: string) {
    return this.http.get<TechRecordSearchSchema[]>(`${environment.VTM_API_URI}/v3/technical-records/search/${term}?searchCriteria=${type}`);
  }
  ...
  testResultsArchiveTestResultIdPut(body: CompleteTestResults, testResultId: string) {
    ...
    return this.http.put<TestResults>(`${environment.VTM_API_URI}/test-results/archive/${encodeURIComponent(String(testResultId))}`, body);
  }
  ...
  testResultsSystemNumberPut(body: CompleteTestResults, systemNumber: string) {
    ...
    return this.http.put<CompleteTestResults>(`${environment.VTM_API_URI}/test-results/${encodeURIComponent(String(systemNumber))}`, body);
  }

  unarchiveTechRecord(systemNumber: string, createdTimestamp: string, reasonForUnarchiving: string, status: string) {
    return this.http.post<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/unarchive/${systemNumber}/${createdTimestamp}`, {
  ...
  updateTechRecord(systemNumber: string, createdTimestamp: string, techRecord: TechRecordType<'put'>) {
    return this.http.patch<TechRecordType<'get'>>(`${environment.VTM_API_URI}/v3/technical-records/${systemNumber}/${createdTimestamp}`, techRecord);
  }
  ...
  updateTechRecords(body: TechRecordPUT, systemNumber: string, oldStatusCode?: string) {
    ...
    return this.http.put<CompleteTechRecordPUT>(`${environment.VTM_API_URI}/vehicles/${encodeURIComponent(String(systemNumber))}`, body, {
      params,
    });
  }
}
