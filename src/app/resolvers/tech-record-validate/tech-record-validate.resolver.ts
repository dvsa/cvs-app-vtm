...
import { VehicleClassDescription as VehicleClassDescriptionPSV } from '@dvsa/cvs-type-definitions/types/v3/tech-record/enums/vehicleClassDescriptionPSV.enum.js';
import { VehicleConfiguration as VehicleConfigurationHgvPsv } from '@dvsa/cvs-type-definitions/types/v3/tech-record/enums/vehicleConfigurationHgvPsv.enum.js';
import { VehicleConfiguration as VehicleConfigurationTrl } from '@dvsa/cvs-type-definitions/types/v3/tech-record/enums/vehicleConfigurationTrl.enum.js';
...
const handlePsv = (record: TechRecordVehicleType<'psv'>) => {
	...
	Object.keys(checks).forEach((key: string) => {
		if (record[key as keyof TechRecordVehicleType<'psv'>]) {
			const validateValues: boolean = Object.values(checks[`${key}`]).includes(
				record[key as keyof TechRecordVehicleType<'psv'>]
			);
...
const handleTrl = (record: TechRecordVehicleType<'trl'>) => {
	...
	Object.keys(checks).forEach((key: string) => {
		if (record[key as keyof TechRecordVehicleType<'trl'>]) {
			const validateValues: boolean = Object.values(checks[`${key}`]).includes(
				record[key as keyof TechRecordVehicleType<'trl'>]
			);
...
const handleHgv = (record: TechRecordVehicleType<'hgv'>) => {
	...
	Object.keys(checks).forEach((key: string) => {
		if (record[key as keyof TechRecordVehicleType<'hgv'>]) {
			const validateValues: boolean = Object.values(checks[`${key}`]).includes(
				record[key as keyof TechRecordVehicleType<'hgv'>]
			);
