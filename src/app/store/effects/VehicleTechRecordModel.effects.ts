...
export class VehicleTechRecordModelEffects {
  ...
  @Effect()
  selectTestType$ = this.actions$.pipe(
    ofType<SelectTestType>(EVehicleTechRecordActions.SelectTestType),
    switchMap(() => {
      return [new SetTestViewState(VIEW_STATE.CREATE)];
    }),
    tap(() => {
      this.router.navigate(['/select-test-type', -1]);
    })
  );
