  ...
  amendVrm$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ newVrm, cherishedTransfer, systemNumber, createdTimestamp, thirdMark }) => {
        return this.httpService.amendTechRecordVrm(newVrm, cherishedTransfer, systemNumber, createdTimestamp, thirdMark).pipe(
  ...
  generateLetter$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([{ letterType, paragraphId }, vehicle, name, email]) =>
        this.httpService.generateLetter(vehicle as TechRecordType<'get'>, letterType, paragraphId, { name, email }).pipe(
