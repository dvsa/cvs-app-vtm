...
import { TechnicalRecordServiceState, initialState } from '../technical-record-service.reducer';
import {
	editingTechRecord,
...
describe('Tech Record Selectors', () => {
	...
	describe('selectTechRecord', () => {
		...
		it.each(routes)(
			'should return the $statusExpected record',
			({ statusExpected, techRecord_createdAt, isEditing, vehicle }) => {
				const mockTechRecord = selectTechRecord.projector(
					{ ...vehicle, techRecord_createdAt } as unknown as TechRecordType<'get'>,
					isEditing,
					{
						systemNumber: 'foo',
						createdTimestamp: 'bar',
