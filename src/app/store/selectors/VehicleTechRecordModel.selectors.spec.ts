...
describe('VehicleTechRecordModel selectors', () => {
  ...
  describe('getCreateState', () => {
    it('should return the current create state', () => {
      const createState = CREATE_STATE.ON;
      expect(getCreateState.projector({ createState })).toEqual(CREATE_STATE.ON);
    });
  });
});
