import {
  Brake, ReferenceDataResourceType, ReferenceDataResourceTypeAudit,
...
const resourceTypeSelector = (resourceType: ReferenceDataResourceType) => createSelector(
...
export const selectReferenceDataByResourceKey = (resourceType: ReferenceDataResourceType, resourceKey: string | number) =>
  createSelector(referenceDataFeatureState, (state) => (isResourceType(resourceType)
    ? state[`${resourceType}`].entities[`${resourceKey}`]
    : undefined));

export const referenceDataLoadingState = createSelector(
  referenceDataFeatureState,
  (state) => Object.values(state).some((feature) => feature.loading),
);
