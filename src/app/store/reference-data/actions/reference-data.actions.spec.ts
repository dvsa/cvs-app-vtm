...
describe('Test Result Actions', () => {
	it('should return correct types', () => {
		...
		expect(fetchReferenceDataByKeySearchSuccess.type).toBe(
			'[API/reference-data] Fetch ResourceType by Key and search Success'
		);
		expect(fetchReferenceDataByKeySearchFailed.type).toBe(
		...
		expect(fetchTyreReferenceDataByKeySearchSuccess.type).toBe(
			'[API/reference-data] Fetch tyre by filter and term Success'
		);
		expect(fetchTyreReferenceDataByKeySearchFailed.type).toBe(
