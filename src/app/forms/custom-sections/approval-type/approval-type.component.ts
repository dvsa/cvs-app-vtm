...
export class ApprovalTypeComponent implements OnInit, OnChanges, OnDestroy {
    ...
    constructor(private dfs: DynamicFormService) { // Inject DatePipe
    }
    ...
    ngOnInit() {
        ...
        // Format techRecord_coifDate
        if (this.techRecord.techRecord_vehicleType == 'psv') {
            // TODO WRONG FORMAT
            if (this.techRecord.techRecord_coifDate) {
                const parts = this.techRecord.techRecord_coifDate.split('-');
                if (parts.length === 3) {
                    const [year, month, day] = parts;
                    this.techRecord.techRecord_coifDate = `${month}/${day}/${year}`;
                }
            }
        }
    }
