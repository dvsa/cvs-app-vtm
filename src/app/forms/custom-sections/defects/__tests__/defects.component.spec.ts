	...
	describe('downloadAllMedia', () => {
		it('should download media from cache if cached media exists', async () => {
			if (component.defectMediaService) {
				const cacheSpy = vi.spyOn(component, 'downloadMediaFromCache').mockImplementation(() => Promise.resolve());
				const httpSpy = vi.spyOn(component, 'downloadMediaFromHttp').mockImplementation(() => Promise.resolve());
				vi.spyOn(component.defectMediaService, 'hasCachedTestResultImages').mockReturnValue(true);
