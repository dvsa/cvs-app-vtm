...
export const ErrorMessageMap: Record<string, (...args: any) => string> = {
  ...
  [ValidatorNames.Max]: (err: { max: number }, label?: string) => `${label || DEFAULT_LABEL} must not be more than ${err.max}`,
  [ValidatorNames.Min]: (err: { min: number }, label?: string) => `${label || DEFAULT_LABEL} must not be less than ${err.min}`,
