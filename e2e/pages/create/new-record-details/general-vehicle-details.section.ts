...
export class GeneralVehicleDetailsSection extends BasePage {
	...
	readonly bodyMakeSelect = new SelectComponent(this.page, 'techRecord_make');
	readonly bodyModelTextInput = new TextInputComponent(this.page, 'techRecord_model');
	...
	async fill(data: Partial<TechRecordType<'put'>>): Promise<void> {
		...
		if (data.techRecord_vehicleType === 'hgv') {
			...
			await this.bodyMakeSelect.fill(data.techRecord_make);
			await this.bodyModelTextInput.fill(data.techRecord_model);
