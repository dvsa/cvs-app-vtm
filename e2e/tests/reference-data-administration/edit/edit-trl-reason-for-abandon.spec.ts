...
test.describe.skip('Edit a TRL reason for abandoning', { tag: ['@reference-data'] }, () => {
	test('A brake code can be amended in the brake codes reference data table', async ({ page }, testInfo) => {
		...
		// check that we can then abandon a TRL test with this amended reason
		// since reasons use the wording as a primary key, expect that the newly worded reason will need reselecting
	});
});
