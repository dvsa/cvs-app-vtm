...
describe('VTM ADR Ticket 1: Updating And Saving Core ADR Details', () => {
  ...
  context('AC4 Technical record DOES already have ADR details on it', () => {
    ...
    it('the Able to carry dangerous goods radio button is already set to yes', () => {
  ...
  context('AC5. User is presented with the applicant details for updating', () => {
    ...
    it(' am able to update the ADR details on this technical record', () => {
    });

    it('I scroll to the ADR details section (to update the ADR details)', () => {
      cy.get('#mat-expansion-panel-header-6 > .mat-content > .mat-expansion-panel-header-title > .item-technical-record')
        .scrollIntoView();
    });

    it('I must first enter the applicant details as per the design (name, street, town, city, post code)', () => {
    ...
    it('I am able to select one vehicle type from the list ', () => {
  ...
  context('AC6. User enters the remaining details for their vehicle type', () => {
    ...
    it('I am updating the ADR details on this technical record', () => {
    });

    it('I have entered the applicant details and I select a vehicle type', () => {
      cy.get('#mat-expansion-panel-header-6 > .mat-content > .mat-expansion-panel-header-title > .item-technical-record')
        .scrollIntoView();
    });

    it('I must complete all mandatory attributes for my vehicle type', () => {
      cy.get('#applicantDetailsCity').type('Dorel Grigore').should('have.value', 'Dorel Grigore');
      cy.get('#applicantDetailsTown').type('Dorel Grigore').should('have.value', 'Dorel Grigore');
      cy.get('#applicantDetailsPostcode').type('Dorel Grigore').should('have.value', 'Dorel Grigore');
    });

    it('I am able to complete any of the optional fields for my vehicle type, if I wish to', () => {
      cy.get('#adrVehicleType').select('Rigid tank').should('have.value', 'Rigid tank');
    });

    it('the attributes that are marked as NA for my vehicle type do not even appear for entry', () => {

    });

    it('I am unable to enter an erroneous date', () => {
